...
test.describe.serial('ENS English local institution admin E2E',()=>{
  ...
  test('A07 catálogo permite crear y editar Learning Unit',async({page})=>{
    ...
    await section.getByLabel('Inglés',{exact:true}).fill('checkpointui');
    await section.getByLabel('Español',{exact:true}).fill('punto de control ui');
    await section.getByLabel('Categoría',{exact:true}).fill('testing');
    ...
    await section.getByLabel('Español',{exact:true}).fill('punto de comprobación ui');
